	...
	$.ajax({
		...
		success:function(data){//data：返回数据（json对象）
			...
			for(var i = 0; i < data.data.length; i++){
				if(cl != null && cl != undefined && data.data[i].id == cl ){
					options += "<option selected=\"selected\" value=\""+data.data[i].id+"\" >"+data.data[i].categoryName+"</option>";
				}else{
...
$(function(){  
	...
	$.ajax({
		...
		success:function(data){//data：返回数据（json对象）
			...
			var options = "<option value=\"\">--请选择--</option>";
			for(var i = 0; i < data.data.length; i++){
				if(fid != null && fid != undefined && data.data[i].valueId == fid ){
					options += "<option selected=\"selected\" value=\""+data.data[i].valueId+"\" >"+data.data[i].valueName+"</option>";
				}else{
